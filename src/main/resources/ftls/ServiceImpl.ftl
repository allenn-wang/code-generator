...
public class ${table.className}${serviceClassName}${serviceImplClassName} extends ${basePackageName?cap_first}${serviceClassName}${serviceImplClassName}<${table.className}, ${table.primaryKeyColumn.javaType}> implements ${table.className}${serviceClassName} {
    ...
    @Override
    public List<${table.className}${dtoClassName}> queryList${dtoClassName}(${table.className}Example ${table.className?uncap_first}Example, Integer pageNum, Integer pageSize) {
        PageHelper.startPage(pageNum,pageSize);
        return this.${table.className?uncap_first}${daoClassName}.queryList${dtoClassName}(${table.className?uncap_first}Example);
    }

    @Override
    public List<${table.className}> queryList(${table.className}Example ${table.className?uncap_first}Example, Integer pageNum, Integer pageSize) {
        PageHelper.startPage(pageNum,pageSize);
        return this.${table.className?uncap_first}${daoClassName}.queryList(${table.className?uncap_first}Example);
    }

    @Override
    public Long count(${table.className}Example ${table.className?uncap_first}Example) {
        return this.${table.className?uncap_first}${daoClassName}.count(${table.className?uncap_first}Example);
    }
